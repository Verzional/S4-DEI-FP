      ...
      <div className="w-full flex flex-col">
        ...
        {showStats && (
          <div className="mb-6 border border-gray-200 rounded-lg p-4 bg-gray-50">
            <h3 className="font-bold text-lg mb-2 text-blue-600">Filler Word Statistics</h3>
            <div className="flex justify-between items-center mb-4">
              <div className="text-gray-700">
                <p>Session time: <span className="font-medium">{sessionTime}s</span></p>
              ...
              <div className="flex gap-1">
                {Object.entries(counts).slice(0, 3).map(([label, count]) => (
                  count > 0 && (
                    <div key={label} className="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm font-medium">
                      {label}: {count}
                    </div>
                  )
                ))}
        ...
        <div className="relative">
          ...
          {isCombinedActive && (
            <div className="bg-green-50 border border-green-200 mt-4 p-4 rounded-lg">
              <h3 className="font-medium mb-1 text-green-800">Pengenalan Suara Aktif</h3>
              <p className="text-green-700 text-sm">
                Bicaralah dengan jelas ke mikrofon Anda. Sistem sedang mendengarkan dan mendeteksi filler words.
              </p>
            </div>
          )}
        </div>
